...
def draw_picture(width, height):
    ...
    # code for BLUE triangles  #Jay
    sg.set_fill_color(colors[2]) # set fill color to red
    
    for i in range(5):   # fill_triangle(x1, y1, x2, y2, x3, y3)
        sg.fill_triangle(2*TRIANGLE_WIDTH, i*TRIANGLE_HEIGHT, 3*TRIANGLE_WIDTH, (i+1)*TRIANGLE_HEIGHT,2*TRIANGLE_WIDTH, (i+1)*TRIANGLE_HEIGHT)
    ...
    for i in range(5):   # fill_triangle(x1, y1, x2, y2, x3, y3)
     ...
     # code for GREEN triangles - Crystal
    sg.set_fill_color("#00ff00") # set fill color to red
